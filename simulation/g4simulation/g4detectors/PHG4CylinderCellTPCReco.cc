...
int PHG4CylinderCellTPCReco::process_event(PHCompositeNode *topNode) {
  ...
  PHG4CylinderCellContainer *cells =
      findNode::getClass<PHG4CylinderCellContainer>(topNode, cellnodename);
  if (!cells) {
    cout << "could not locate cell node " << cellnodename << endl;
    exit(1);
  }
  PHG4CylinderCellGeomContainer *seggeo =
      findNode::getClass<PHG4CylinderCellGeomContainer>(topNode,
                                                        seggeonodename.c_str());
  if (!seggeo) {
  ...
  pair<PHG4HitContainer::LayerIter, PHG4HitContainer::LayerIter>
      layer_begin_end = g4hit->getLayers();

  for (layer = layer_begin_end.first; layer != layer_begin_end.second;
       layer++) {
    ...
    for (hiter = hit_begin_end.first; hiter != hit_begin_end.second; hiter++) {
      ...
      if ((*layer) < (unsigned int)num_pixel_layers) {
        // inner silicon layers

        int phibin = geo->get_phibin(
            atan2(hiter->second->get_y(0), hiter->second->get_x(0)));
        if (phibin < 0 || phibin >= nphibins) continue;

        int zbin = geo->get_zbin(hiter->second->get_z(0));
        ...
        if (cellptmap.count(key) > 0) {
          cellptmap.find(key)->second->add_edep(hiter->first,
                                                hiter->second->get_edep());
          cellptmap.find(key)->second->add_shower_edep(
              hiter->second->get_shower_id(), hiter->second->get_edep());
        } else {
          ...
          it->second->add_edep(hiter->first, hiter->second->get_edep());
          it->second->add_shower_edep(hiter->second->get_shower_id(),
                                      hiter->second->get_edep());
        }
        ...
        if (distortion) {
          ...
          //      because it leads to major change to the structure of this
          //      code and it affect the insensitive direction to
          //      near radial tracks
          //
          //          const double dr = distortion
          //          ->get_r_distortion(r,phi,z);
          phi += drphi / r;
          z += dz;
        }

        if (hiter->second->get_z(0) >= 0.0) {
          z -= driftv * hiter->second->get_t(0);
	  if (z < 0.0) continue;
	  if (z > (geo->get_zmin() + geo->get_zbins()*geo->get_zstep())) continue;	  
        } else {
          z += driftv * hiter->second->get_t(0);
	  if (z > 0.0) continue;
	  if (z < geo->get_zmin()) continue;
	}
	...
	if (hiter->second->get_z(0) >= 0.0)
	  ...
	  zbin = geo->get_zbin(hiter->second->get_z(0)
        ...
        double cloud_sig_x =
            1.5 * sqrt(diffusion * diffusion *
                           (100. - TMath::Abs(hiter->second->get_z(0))) +
                       0.03 * 0.03);
        double cloud_sig_z =
            1.5 * sqrt((1. + 2.2 * 2.2) * diffusion * diffusion *
                           (100. - TMath::Abs(hiter->second->get_z(0))) +
                       0.01 * 0.01);
        ...
        for (int iphi = -n_phi; iphi <= n_phi; ++iphi) {
          ...
          double phi_integral =
              0.5 * erf(-0.5 * sqrt(2.) * phidisp * r * cloud_sig_x_inv +
                        0.5 * sqrt(2.) *
                            ((0.5 + (double)iphi) * phistepsize * r) *
                            cloud_sig_x_inv) -
              0.5 * erf(-0.5 * sqrt(2.) * phidisp * r * cloud_sig_x_inv +
                        0.5 * sqrt(2.) *
                            ((-0.5 + (double)iphi) * phistepsize * r) *
          ...
          for (int iz = -n_z; iz <= n_z; ++iz) {
            ...
            if ((cur_z_bin < 0) || (cur_z_bin >= nzbins)) {
              continue;
            }

            double z_integral =
                0.5 * erf(-0.5 * sqrt(2.) * zdisp * cloud_sig_z_inv +
                          0.5 * sqrt(2.) * ((0.5 + (double)iz) * zstepsize) *
                              cloud_sig_z_inv) -
                0.5 * erf(-0.5 * sqrt(2.) * zdisp * cloud_sig_z_inv +
                          0.5 * sqrt(2.) * ((-0.5 + (double)iz) * zstepsize) *
                              cloud_sig_z_inv);
            ...
            double total_weight =
                rand.Poisson(nelec * (phi_integral * z_integral));
            ...
            if (cellptmap.count(key) > 0) {
              ...
              cellptmap.find(key)->second->add_shower_edep(
                  hiter->second->get_shower_id(), total_weight);
            } else {
              cellptmap[key] = new PHG4CylinderCellv1();
              std::map<std::string, PHG4CylinderCell *>::iterator it =
                  cellptmap.find(key);
    ...
    int count = 0;
    for (std::map<std::string, PHG4CylinderCell *>::iterator it =
             cellptmap.begin();
         it != cellptmap.end(); ++it) {
