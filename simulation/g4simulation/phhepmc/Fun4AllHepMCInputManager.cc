...
int
Fun4AllHepMCInputManager::fileopen(const string &filenam)
{
  if (!mySyncManager)
    {
...
int 
Fun4AllHepMCInputManager::run(const int nevents)
{
 readagain:
  if (!isopen)
    {
...
int
Fun4AllHepMCInputManager::fileclose()
{
  if (!isopen)
    {
...
void
Fun4AllHepMCInputManager::Print(const string &what) const
{
...
int
Fun4AllHepMCInputManager::OpenNextFile()
{
...
int
Fun4AllHepMCInputManager::PushBackEvents(const int i)
{
...
Fun4AllHepMCInputManager::ConvertFromOscar()
{
  ...
  evt = new HepMC::GenEvent(momentumunit, lengthunit);
